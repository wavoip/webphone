...
| Seção                       | Conteúdo                                                                                                                                                                                                         |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Estatísticas em tempo real** | Atualizadas a cada novo evento `stats` e `serverStats`. Por grupo: **RTT** mín/méd/máx (ms). **TX** e **RX**: pacotes (`pkt`), bytes acumulados (`B`), bitrate instantâneo (`kbps`), nível de áudio RMS 0–1 (`lvl`) e perda %. **RX** também expõe `jitter` (ms) — variância de chegada dos pacotes (RTCP para chamadas oficiais, inter-arrival RFC 3550 para chamadas via WebSocket). **audio** mostra `out lat` (`AudioContext.outputLatency`, ms). **server RTT** apresenta as duas pernas (`client` servidor↔cliente e `whatsapp` servidor↔WhatsApp). |
